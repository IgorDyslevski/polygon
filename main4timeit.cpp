...
int main() {
    ...
    for (int i = 1; i < 26; i++) {
        ...
        std::vector<int> tofill = {1, 1};
        for (int j = 0; j < i; j++) {
            data.push_back(tofill);
        }
        int wlimit = i;
        clock_t start = clock();
        int price_max_dec = find_dec_mask(data, wlimit);
        clock_t end = clock();
        std::cout << i << ": " << (long double)(end - start) / (long double)(CLOCKS_PER_SEC) << "," << std::endl;
        // std::cout << (long double)(end - start) / (long double)(CLOCKS_PER_SEC) << " ";
